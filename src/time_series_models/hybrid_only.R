...
# setwd("C:/Users/snallusamy/Desktop/Forecasting Project/OneDrive_1_5-31-2017/Data Files/Excel_Files")
# 
# #load data
...
# dat <- subset(data, PlantNumber == 2530)
# #new Table carrying 4 variables

hybrid <- function(dat, inter, prodlevel, custlevel, start_date, end_date, vol, hor, options){
  
  # inter = "Week"
  # prodlevel = "DieNumber"
  # custlevel = "SoldTo"
  # start_date = as.Date("2014-10-26",format = "%Y-%m-%d")
  # end_date = as.Date("2017-01-15",format = "%Y-%m-%d")
  ...
  ##First Calculating ETS function
  etsFit <- 1
  horizon <-  hor
  Jan_etsfc <- matrix(1,horizon,ncol(my_ts))
  etsfitt <- matrix(1,nrow(my_ts),ncol(my_ts))
  
  for (i in 1:ncol(my_ts)){
    etsFit <- ets(my_ts[,i])
    etsfitt[,i] <- as.numeric(etsFit$fitted)
    etsForecast <- forecast(etsFit, h = horizon)
    Jan_etsfc[,i] <- as.numeric(etsForecast$mean)
    for(j in 1:horizon){
      if(Jan_etsfc[j,i] < 0)
        Jan_etsfc[j,i] = 0
      else(Jan_etsfc[j,i] = Jan_etsfc[j,i])
    }
  }
  ...
  arimaFit <- 1
  horizon <-  hor
  Jan_arimafc <- matrix(1,horizon,ncol(my_ts))
  arimafitt <- matrix(1,nrow(my_ts),ncol(my_ts))
  
  
  for (i in 1:ncol(my_ts)){
    arimaFit <- auto.arima(my_ts[,i])
    arimafitt[,i] <- as.numeric(arimaFit$fitted)
    arimaForecast <- forecast(arimaFit, h = horizon)
    Jan_arimafc[,i] <- as.numeric(arimaForecast$mean)
    for(j in 1:horizon){
      if(Jan_arimafc[j,i] < 0)
        Jan_arimafc[j,i] = 0
      else(Jan_arimafc[j,i] = Jan_arimafc[j,i])
    }
  }
  
  df2[is.na(df2)] <- 0
  monthactuals <- df3
  #Averaging forecasts and Fitt from 2 functions
  monthforecast_auto <- (etsfitt+arimafitt)/2
  Jan_autofc <- (Jan_arimafc+Jan_etsfc)/2
  
  monthAbsErr_auto <- matrix(1,nrow(monthactuals),ncol(monthactuals))
  APEauto_Jan <- matrix(1,nrow(monthactuals),)
  
  for (i in 1:nrow(monthactuals))
    for (j in 1:ncol(monthactuals))
      monthAbsErr_auto[i,j] <- abs(as.numeric(monthactuals[i,j]) - as.numeric(monthforecast_auto[i,j]))
  for(i in 1:nrow(monthactuals))
    APEauto_Jan[i,] <- sum(monthAbsErr_auto[i,])/sum(as.numeric(monthactuals[i,]))
  
  Apeauto_avg <- mean(APEauto_Jan)
  
  Accuracy <- 1-Apeauto_avg
  
  Actuals <- rowSums(df2[v1:(v2 + horizon),], na.rm= TRUE)
  act <- data.frame(Actuals)
  Forecasts <- rowSums(rbind(monthforecast_auto,Jan_autofc), na.rm=TRUE)
  ...
  a <- ggplot(p) +
    ...
    scale_colour_manual(values=c("Red","Blue"))  + theme_light()+ theme(legend.text=element_text(size=12))
