...
# -----------------Below section can be uncommented for individual analysis  ----------- # 
# ----------------------------------------  -------------------------------------------------- # 
...
#Simple Function contianing all the data from the input data

simple <- function(dat, inter, prodlevel, custlevel, start_date, end_date, vol, hor, ord, options){

#Choosing only 4 variables for combinging the product-customer variables  
...
## These are the local variables created based on the time series chosen!
  #For Interval
  if(inter == "Month"){
      freqq = 12
      d = 31
...
if(inter == "Quarter"){
  ...
  #Spreading Combination as columns and time series as rows
  feed_data <- spread(dat, Combination,cartons_sum)
  wee <- feed_data[,1]

  #Capturing numbers before and after logic for indexing
  new1 <- length(feed_data$Interval)+1
  old1 <- length(feed_data$Interval)+0
  
  # Removing the time series for better handling the calculation
  feed_data <- subset(feed_data, select = -Interval)

  #logic to categorize SKUs 
  ...
  #removing Single occurance SKUs
  df1 <- df[,!sapply(df, function(x) sum(!is.na(x)) < 2 )]

  #data frame chosen based on the SKU frequency selection 
  ...
  #Choosing only the required rows
  df2 <- df2[1:old1,]
  df2[is.na(df2)] <- 0
  ...
  #This index chooses the starting and the ending datees
  df3 <- df2[v1:v2,]

  ##This block chooses the right ts() based on the time series chosen
...
#SMA modeling algorihm
  ...
  # Fetchng actuals and forecasts+fitted for plotting 
